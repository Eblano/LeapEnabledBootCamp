...
public class CharacterMotorMovement : object
{
    ...
    public AnimationCurve maxSpeedOnSlope;
    ...
    public CharacterMotorMovement()
    {
        ...
        this.maxSpeedOnSlope = new AnimationCurve(new Keyframe[] {new Keyframe(-90, 10), new Keyframe(0, 10), new Keyframe(90, 0)});
...
public partial class CharacterMotor : MonoBehaviour
{
    ...
    private Vector3 GetDesiredHorizontalVelocity()
    {
        ...
        if ((this.movement.clampSpeedOnSlopes && this.grounded) && (this.movement.velocity.sqrMagnitude > ((maxSpeed * maxSpeed) * 0.0001f)))
        {
            float movementSlopeAngle = Mathf.Asin(this.movement.velocity.normalized.y) * Mathf.Rad2Deg;
            float groundSlopeAngle = Mathf.Acos(this.groundNormal.y) * Mathf.Rad2Deg;
            if (Mathf.Abs(movementSlopeAngle) > groundSlopeAngle)
            {
                movementSlopeAngle = groundSlopeAngle * Mathf.Sign(movementSlopeAngle);
            }
            maxSpeed = Mathf.Clamp(maxSpeed, 0, this.movement.maxSpeedOnSlope.Evaluate(movementSlopeAngle));
        }
